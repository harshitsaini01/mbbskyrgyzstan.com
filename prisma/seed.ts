...
async function main() {
    ...
    const [publicType, privateType] = await Promise.all([
        prisma.instituteType.upsert({ where: { id: 1 }, update: {}, create: { name: "Public", status: true } }),
        prisma.instituteType.upsert({ where: { id: 2 }, update: {}, create: { name: "Private", status: true } }),
    ]);
    ...
    for (const ud of uniSeed) {
        let uni = await prisma.university.findUnique({ where: { slug: ud.slug } });
        if (!uni) {
            uni = await prisma.university.create({ data: ud as any });
        } else {
            uni = await prisma.university.update({ where: { slug: ud.slug }, data: ud as any });
        }
