...
export default function EditArticlePage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!form) return;
        setLoading(true);
        const res = await fetch(`/api/admin/articles/${id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) });
        if (res.ok) { toast.success("Updated!"); router.push("/admin/articles"); }
    ...
    return (
        <div className="max-w-3xl mx-auto space-y-6">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <ImageUpload label="Thumbnail" value={form.thumbnailPath as string | null} onChange={(v) => set("thumbnailPath", v)} folder="articles" />
                            <ImageUpload label="Featured Image" value={form.imagePath as string | null} onChange={(v) => set("imagePath", v)} folder="articles" />
                        </div>
                    </div>
            ...
            {activeTab === "Contents" && (
                <div className="bg-white rounded-xl border border-gray-200 p-6">
                    <ContentSectionsTab entityId={id} apiBase={`/api/admin/articles/${id}/contents`} />
                </div>
            )}
