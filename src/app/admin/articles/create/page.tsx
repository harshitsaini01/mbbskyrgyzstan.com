...
type FormData = {
    ...
    status: boolean; homeView: boolean; trending: boolean;
...
export default function CreateArticlePage() {
    ...
    const [form, setForm] = useState<FormData>({ title: "", slug: "", categoryId: "", shortnote: "", description: "", thumbnailPath: null, imagePath: null, status: true, homeView: false, trending: false, metaTitle: "", metaKeyword: "", metaDescription: "" });
    const [loading, setLoading] = useState(false);

    useEffect(() => { fetch("/api/admin/article-categories").then(r => r.json()).then(setCategories); }, []);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (res.ok) { toast.success("Article published!"); router.push("/admin/articles"); }
    ...
    return (
        <div className="max-w-3xl mx-auto space-y-6">
            <div className="flex items-center gap-4">
                <Button variant="ghost" size="icon" asChild><Link href="/admin/articles"><ArrowLeft size={18} /></Link></Button>
                <div><h1 className="text-2xl font-bold text-gray-900">New Article</h1><p className="text-sm text-gray-500">Write a blog article</p></div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <ImageUpload label="Thumbnail" value={form.thumbnailPath} onChange={(v) => set("thumbnailPath", v)} folder="articles" />
                        <ImageUpload label="Featured Image" value={form.imagePath} onChange={(v) => set("imagePath", v)} folder="articles" />
                    </div>
                </div>
                ...
                <div className="flex justify-end gap-3 pb-8">
                    <Button variant="outline" type="button" asChild><Link href="/admin/articles">Cancel</Link></Button>
